...
import { ConfirmSeatNotes } from './confirmCardParts';
...
function WideCard({ model, props }: Readonly<{ model: SeatLayerPickerConfirmationModel; props: SeatLayerPickerSeatConfirmationProps }>): React.ReactElement {
  ...
  // §3.8.9 — the tap card is the COMPACT form of the one row model. It used
  // to keep its own subset (one limited-view line, a wheelchair notice titled
  // "Accessibility and view", the organizer's sentence with no mark to explain
  // it), which is exactly the drift the model exists to end.
  ...
  return <View style={nativeStyles.center}><View onLayout={(event) => setParentWidth(layoutWidth(event))} style={[nativeStyles.hit, { maxHeight: Math.max(0, height * .72) }]}><View style={[nativeStyles.card, { backgroundColor: scope.resolvedTheme.colors.surface, borderColor: scope.resolvedTheme.colors.divider, borderRadius: seatLayerPickerTokens.radius.card }, styles.confirmCardContainer, sanitizeSeatLayerPickerStyle(props.style)]}>
    <ScrollView contentContainerStyle={nativeStyles.scroll}>
      ...
      {/* Full-bleed BANDS, so they sit outside the body's own gutter and
          directly under the category band. */}
      <ConfirmSeatNotes compact fontFamily={scope.resolvedTheme.fontFamily} palette={notePalette} rows={notes} />
...
const nativeStyles = seatLayerPickerBoldStyles(StyleSheet.create({
  center: { alignItems: 'center', justifyContent: 'center', padding: 12 }, hit: { width: '100%', maxWidth: 430, minHeight: seatLayerPickerTokens.size.minimumHitTarget }, card: { borderWidth: seatLayerPickerLineWidth, overflow: 'hidden', maxHeight: '100%' }, scroll: { flexGrow: 1 }, grid: { borderBottomWidth: seatLayerPickerLineWidth }, field: { flex: 1, minHeight: 62, padding: 10, borderEndWidth: seatLayerPickerLineWidth, justifyContent: 'center' }, fieldLabel: { fontSize: 11, fontWeight: '800' }, fieldValue: { fontSize: 15, fontWeight: '900' }, categoryBand: { minHeight: 58, alignItems: 'center', gap: 10, paddingHorizontal: 16, borderBottomWidth: seatLayerPickerLineWidth }, categoryDot: { width: 14, height: 14, borderRadius: 7, borderWidth: 1 }, categorySpacer: { flex: 1 }, categoryLabel: { flex: 1, fontSize: 16, fontWeight: '800' }, price: { fontSize: 19, fontWeight: '900' }, body: { gap: 10, padding: 16 }, inspection: { gap: 8 }, actions: { gap: 10 }, actionHit: { flex: 1, minHeight: seatLayerPickerTokens.size.minimumHitTarget, justifyContent: 'center' }, actionPaint: { height: seatLayerPickerTokens.size.confirmActionHeight, borderWidth: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 8 }, actionText: { fontSize: 14, fontWeight: '800' },
}));
