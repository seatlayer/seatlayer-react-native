  ...
  >
    ...
    >
      <View pointerEvents="none" style={[
        styles.captionStrip,
        { backgroundColor: seatLayerPickerColorAlpha(props.theme.colors.surface, 0.92), borderColor: props.theme.colors.divider, flexDirection: rtl ? 'row-reverse' : 'row' },
        slots?.seatViewChromeContainer,
        sanitizeSeatLayerPickerStyle(props.style),
      ]}>
        <View pointerEvents="none" style={styles.copy}>
          {wording.title ? <Text numberOfLines={2} style={[styles.title, { color: props.theme.colors.text, fontFamily: props.theme.fontFamily, textAlign: rtl ? 'right' : 'left' }]}>{wording.title}</Text> : null}
          {wording.caption ? <Text numberOfLines={2} style={[styles.caption, { color: props.theme.colors.mutedText, fontFamily: props.theme.fontFamily, textAlign: rtl ? 'right' : 'left' }]}>{wording.caption}</Text> : null}
        </View>
        {wording.badge ? <View pointerEvents="none" style={[
          styles.badge,
          props.view.real
            ? { backgroundColor: props.theme.colors.accent }
            : { backgroundColor: seatLayerPickerColorAlpha(props.theme.colors.text, 0.14) },
          slots?.seatViewChromeButton,
        ]}>
          <Text numberOfLines={1} style={[
            styles.badgeText,
            { color: props.view.real ? props.theme.colors.onAccent : props.theme.colors.text, fontFamily: props.theme.fontFamily, textAlign: rtl ? 'right' : 'left' },
            slots?.seatViewChromeButtonText,
          ]}>{wording.badge}</Text>
        </View> : null}
      </View>
      {props.showDragHint === true && wording.dragHint ? <View pointerEvents="none" style={[styles.hintChip, { backgroundColor: seatLayerPickerColorAlpha(props.theme.colors.surface, 0.8) }]}>
        <Text numberOfLines={1} style={[styles.hint, { color: props.theme.colors.mutedText, fontFamily: props.theme.fontFamily, textAlign: rtl ? 'right' : 'left' }]}>{wording.dragHint}</Text>
      </View> : null}
    </View>
  </View>;
}
...
const styles = {
  ...
  caption: { fontSize: 12, fontWeight: '600', lineHeight: 17, marginTop: 3 } as TextStyle,
  captionStrip: { alignItems: 'flex-start', alignSelf: 'stretch', borderRadius: seatLayerPickerTokens.radius.button, borderWidth: 1, gap: 10, maxWidth: '100%', paddingHorizontal: 12, paddingVertical: 10 } as ViewStyle,
  ...
  hint: { fontSize: 12, fontWeight: '600' } as TextStyle,
  ...
  title: { fontSize: 14, fontWeight: '800', lineHeight: 18 } as TextStyle,
} as const;
